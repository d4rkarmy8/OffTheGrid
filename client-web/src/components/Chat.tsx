        ...
        <div className="flex h-screen w-full bg-surface-950 text-white overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto">
                    ...
                    {filteredInbox.map((conv, i) => (
                        <div
                            ...
                            className={`p-3.5 mx-2 my-0.5 rounded-xl cursor-pointer transition-all duration-200 animate-fade-in ${activeContact === conv.contact
                        ...
                        >
                            <div className="flex items-center gap-3">
                                <div className="relative flex-shrink-0">
                                    ...
                                    <span className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-surface-900 ${isContactOnline(conv.contact) ? 'bg-green-500' : 'bg-surface-200/20'}`} />
                                </div>
            ...
            <div className="flex-1 flex flex-col bg-surface-950">
                {activeContact ? (
                    <>
                        {/* Chat header */}
                        <div className="px-6 py-4 border-b border-white/6 bg-surface-900/50 backdrop-blur-xl flex justify-between items-center">
                            <div className="flex items-center gap-3">
                                <div className="relative">
                                    ...
                                    <span className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-surface-900 ${isContactOnline(activeContact) ? 'bg-green-500' : 'bg-surface-200/20'}`} />
                                </div>
                                <div>
                                    <h3 className="font-semibold text-sm">{activeContact}</h3>
                                    <span className={`text-xs ${isContactOnline(activeContact) ? 'text-green-400' : 'text-surface-200/40'}`}>
                        ...
                        <div className="flex-1 overflow-y-auto p-6 space-y-3">
                            ...
                            {messages.map((msg, i) => {
                                ...
                                return (
                                    <div
                                        key={msg.id || i}
                                        className={`flex ${isMe ? 'justify-end' : 'justify-start'} ${isMe ? 'animate-slide-right' : 'animate-slide-left'}`}
                                    >
                                        <div
                                        ...
                                        >
                                            ...
                                            <div className={`text-[10px] mt-1 flex items-center gap-1 ${isMe ? 'text-primary-200/60 justify-end' : 'text-surface-200/30'}`}>
